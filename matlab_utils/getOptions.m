...
% Example usage by RC:
%     if nargin>=4
%     options=imported.biafra.getOptions(options,varargin{3:end});
%       end
%
% another example use by RC
% function(arg1, 'options',options_structure)
% options=imported.biafra.getOptions(options,{'options',options_structure});
    ...
    % 2020.06.03 [13:43:14] - parsing options as a structure passed
    % straight as an argument rather than 'options', options value. (by
    % Radek Chrapkiewicz)
    % TODO
    ...
    % recognizing if options have been all passed as a structure (2020-06-03 13:36:09 RC) 
    %     - parsing options as a structure passed
    % straight as an argument rather than 'options', options value. (by
    % Radek Chrapkiewicz)
    %
    % TODO - what if parameter, values are passed after options structure ?
    if isstruct(inputArgs)
        inputArgs={'options',inputArgs};
    end
    if ~isempty(inputArgs)
        if isstruct(inputArgs{1}) % next arguments will be ignored
            inputArgs={'options',inputArgs{1}};
        end
    end
