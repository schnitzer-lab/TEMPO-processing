...
files = [dir(basefolder_analysis + "\Isofluorane\m45\20211005\meas*")]; %dir(basefolder_raw + "Visual\m40\20210824\meas00\");
...
for i_f = 1:length(recording_names)
    ...
    try
        ...
        % movieCopyReference(fullpath1, fullpath1_reference);
        % movieCopyReference(fullpath2, fullpath2_reference);
        movieCopyReference(fullpath1, []);
        movieCopyReference(fullpath2, []);
        
        fullpath1 = movieRemoveOutlierFrames(fullpath1);
        fullpath2 = movieRemoveOutlierFrames(fullpath2);
