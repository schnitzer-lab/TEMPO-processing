function fullpath_out_all = movieExtractCustomOutlinesTrace(fullpath_movie, contour_ids, varargin)
    ...
    if(~iscell(contour_ids)), contour_ids = num2cell(contour_ids); end
    %%
    ...
    if (all(isfile(fullpath_out_all)))
        if(options.skip)
            displog("All output files exist. Skipping: " + strjoin(fullpath_out_all, ', '))
            return;
        else
    ...
    if(isempty(specs.getCustomOutlines()))
        error("movieExtractCustomOutlinesTrace: no custom outlines found: " + fullpath_movie)
    end
    if(isempty(specs.getMask()))
        warning("movieExtractCustomOutlinesTrace: no mask found: " + fullpath_movie)
    end
    %%
    ...
    for i_r = 1:length(contour_ids)
    %% 
    
        region_id = contour_ids{i_r};
        ...
        for i_c = 1:length(region_id)
            contours{i_c} = specs.getCustomOutlines(region_id(i_c));
        end
        ...
        fig_traces = plt.getFigureByName("movieExtractCustomOutlinesTrace: mean traces");
