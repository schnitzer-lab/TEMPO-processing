...
%PMTM  Estimate power spectral density via the multitaper method using 
%   Slepian or sine tapers.
%   Pxx = PMTM(X) returns the power spectral density (PSD) estimate, Pxx,
%   of a discrete-time signal X using Slepian tapers. When X is a vector,
%   it is converted to a column vector and treated as a single channel.
%   When X is a matrix, the PSD is computed independently for each column
%   and stored in the corresponding column of Pxx. Pxx is the distribution
%   of power per unit frequency. The frequency is expressed in units of
%   radians/sample.
%
...
%
%   Pxx = PMTM(X,'Tapers',TYPE) returns the PSD estimate, Pxx, of a signal
%   X using specified tapers, where TYPE is one of ['slepian' | 'sine'].
%   If omitted, 'Tapers' defaults to 'slepian'.
%
%   Pxx = PMTM(X,NW) specifies NW as the time-bandwidth product for the
%   discrete prolate spheroidal (Slepian) sequences used as data windows.
%   Typical choices for NW are 2, 5/2, 3, 7/2, or 4. If empty or omitted,
%   NW defaults to 4. By default, PMTM drops the last taper because its
%   corresponding eigenvalue is significantly smaller than 1. Therefore,
%   the number of tapers used to form Pxx is 2*NW-1.
%
%   Pxx = PMTM(X,M,'Tapers','sine') specifies M as the number of sine
%   tapers used as data windows or as the weights used for weighted
%   averaging when generating the PSD estimate. If empty, M defaults to 7
%   with uniform weighting. When M is a vector, the number of weights is
%   equal to the number of tapers used. The sum of the weights must be
%   equal to 1.
%
%   Pxx = PMTM(...,NFFT) specifies the FFT length used to calculate the PSD
%   estimate. For real X, Pxx has (NFFT/2+1) rows if NFFT is even and
%   (NFFT+1)/2 rows if NFFT is odd. For complex X, Pxx always has NFFT
%   rows. If NFFT is specified as empty, NFFT is set to either 256 or the
%   next power of 2 greater than the length of X, whichever is larger.
%
%   [Pxx,W] = PMTM(...,NFFT) returns the vector of normalized angular
...
%
%   [Pxx,W] = PMTM(...,W) computes the two-sided PSD at the normalized
%   angular frequencies contained in vector W. W must have at least two
%   elements.
%
%   [Pxx,F] = PMTM(...,NFFT,Fs) returns the PSD computed as a function of
%   physical frequency. Fs is the sample rate specified in hertz.
%   If Fs is empty, it defaults to 1 Hz.
%
%   F is the vector of frequencies (in hertz) at which the PSD is
%   estimated. For real signals, F spans the interval [0,Fs/2] when NFFT is
%   even and [0,Fs/2) when NFFT is odd. For complex signals, F always
%   spans the interval [0,Fs).
%
%   [Pxx,F] = PMTM(...,F,Fs) computes the two-sided PSD at the frequencies
...
%   default value for P is 0.95. Confidence intervals are computed using a
...
%
%   [...] = PMTM(...,FREQRANGE) returns the PSD over the specified range of
%   frequencies based upon the value of FREQRANGE:
%
%      'onesided' - returns the one-sided PSD of a real input signal X.
%         If NFFT is even, Pxx has NFFT/2+1 rows and is computed over the
%         interval [0,pi]. If NFFT is odd, Pxx has (NFFT+1)/2 rows and
...
%         input X. Pxx has NFFT rows and is computed over the interval
%         [0,2*pi). When Fs is specified, the interval becomes [0,Fs).
%
%      'centered' - returns the centered two-sided PSD for either real or
%         complex X. Pxx has NFFT rows and is computed over the interval
...
%
%   The following syntaxes apply to Slepian tapers only:
%
%   [Pxx,F] = PMTM(...,'DropLastTaper',DROPFLAG) specifies whether PMTM
%   should drop the last taper/eigenvector during the calculation. 
%   DROPFLAG can be one of the following values: [ true | false ].
%       true  - the last taper/eigenvector is dropped (default).
...
%
%   [Pxx,F] = PMTM(X,E,V,NFFT,Fs,METHOD) returns the PSD estimate and
%   frequency vector from the data tapers in E and their concentrations V.
%   Type HELP DPSS for a description of the matrix E and the vector V. By
%   default, PMTM drops the last eigenvector because its corresponding
%   eigenvalue is significantly smaller than 1.
%
%   [Pxx,F] = PMTM(X,DPSS_PARAMS,NFFT,Fs,METHOD) uses the cell array
%   DPSS_PARAMS containing the input arguments to DPSS (listed in order,
%   but excluding the first argument) to compute the data tapers. For
%   example, PMTM(x,{3.5,'trace'},512,1000) calculates the prolate
...
%      Fs = 1000;   t = 0:1/Fs:.3; 
%      x = cos(2*pi*t*200)+randn(size(t)); % A cosine of 200Hz plus noise
%      pmtm(x,5,[],Fs,'Tapers','sine');    % Uses sine tapers
%
...
%     [3] Riedel, K.S. and Sidorenko, A. "Minimum bias multiple taper
%         spectral estimation." IEEE Trans. Signal Process., Vol. 43
%         (1995), pp. 188–195.
...
validateattributes(x,{'single','double'}, {'finite','nonnan'},'pmtm','x')
...
if params.tapersType == "sine" && ~isempty(indx)
     error(message('signal:pmtm:InvalidInputSineMTM', 'Droplasttaper'));
end
...
if ~strcmp(params.conflevel,'omitted') && (nargout==0 || nargout>2)
  ...
  Nchan = size(x,2);
  c = signal.internal.spectral.chi2conf(params.conflevel,k);
...
elseif (~strcmp(params.ConfInt,'omitted') && nargout==0) || nargout>2
  ...
  if ~strcmp(params.ConfInt,'omitted')
    c = signal.internal.spectral.chi2conf(params.ConfInt,k);
  else % use default value of .95
    c = signal.internal.spectral.chi2conf(.95,k);
  end      
  Nchan = size(x,2);
  Pxxc(:,2:2:2*Nchan) = double(Pxx)*c(2);
  Pxxc(:,1:2:2*Nchan) = double(Pxx)*c(1);
...
% center dc if needed
if params.centerdc
   [Pxx, f, Pxxc] = signal.internal.spectral.psdcenterdc(Pxx, f, Pxxc, params);
end
...
for chan=1:Nchan
    ...
    case 'adapt'
       ...
       while sum(abs(Schan-S1)/nfft_mod)>tol
          ...
          if(iteration >= max_iteration)
              warning("adaptive pmtm did not converge")
              break;
          end
    ...
    case {'unity','eigen'}
       ...
       % When using sine tapers we will get sum(wt)=1
       if params.tapersType == "sine"
           Schan = Sk*wt;
       else
           Schan = Sk*wt/k;
       end
...
% Parse the input arguments to get the tapers type if specified
tapersParmsIdx = strcmpi('tapers',varargin);
tapersIdx = find(tapersParmsIdx ==1);
if ~isempty(tapersIdx)
    tapersType = varargin{tapersIdx+1};
    if ~any(strcmp(tapersType,{'sine','slepian'}))
        error(message('signal:pmtm:InvalidTapersValue'));
    end
    tapersParmsIdx(tapersIdx+1) = true;
    % Remove tapersParams from varargin
    varargin = varargin(~tapersParmsIdx);
else
    tapersType = 'slepian';
end
...
if tapersType == "sine"
    indx = 1;
    numTapers = 7;
    weights = (1/numTapers).* ones(numTapers,1);
    nfft_temp = [];
    if ~isempty(varargin) && ~ischar(varargin{1})
        indx = 2;
        if isscalar(varargin{1})
            validateattributes(varargin{1},{'double','single'},{'scalar','finite','nonnan','nonempty','positive','integer'},'PMTM','M');
            numTapers = varargin{1};
            if numTapers==1
                error(message('signal:pmtm:insufficientNumberOfTapers','M'));
            end
            weights = (1/numTapers).* ones(numTapers,1);
        elseif ~isempty(varargin{1})
            validateattributes(varargin{1},{'double','single'},{'vector','finite','nonnan','nonempty'},'PMTM','Weights');
            if abs(sum(varargin{1})-1) > sqrt(eps)
                error(message('signal:pmtm:InvalidWeights'));
            end
            weights = varargin{1};
            numTapers = numel(weights);
            if numTapers==1
                error(message('signal:pmtm:insufficientNumberOfWeights','Weights'));
            end
        end
        % If we have more than 2 element in varargin after removing 'Taper'
        % key-value then try to find Nfft
        if length(varargin) > 1
            nfft_temp = findNFFT(varargin{2:end});
        end
    end
    dataPrecision = class(x);
    E = signal.internal.sinetapers(N,numTapers,dataPrecision);
    V = cast(weights,dataPrecision);
    NW = [];
else
...
if ~isempty(params.MTMethod)
    if tapersType == "sine"
        error(message('signal:pmtm:InvalidInputSineMTM', params.MTMethod));
    end
else
    if tapersType == "sine"
        params.MTMethod = 'eigen';
    else
